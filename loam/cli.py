"""Loam's command line — how you tend and inhabit the world.
  ...
  loam run --ticks 20            # let it live 20 ticks (mock cognition, free)
...
from .model import ClaudeModel, MockModel
from .world import World


def _make_model(real: bool):
    return ClaudeModel() if real else MockModel()


def _load_or_make(real: bool, agents: int, seed: int) -> World:
    model = _make_model(real)
    w = persistence.load(model=model)
    if w is None:
...
def cmd_run(args: argparse.Namespace) -> int:
    ...
    persistence.save(w)
    new = w.feed[before:]
    tail = new if new else w.feed[-10:]
    print(f"— {args.ticks} ticks lived (now t{w.tick}, {len(w.agents)} beings) —")
    for line in tail[-16:]:
        print(line)
    return 0
...
def main(argv: list[str] | None = None) -> int:
    # The world speaks in symbols; make sure the terminal can render them.
